...
export default function WaitlistCTA({ eventId, tierId, defaultEmail, defaultName, accentStyle }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast({ kind: 'success', title: t('waitlist.successTitle'), message: t('waitlist.success', { email: trimmed }) });
    } catch (err: any) {
  ...
  if (position !== null) {
    return (
      <div className="w-full border-2 border-brand-primary/40 bg-brand-primary/10 p-5 text-center">
        ...
        <p className="uppercase tracking-tighter italic text-sm font-black">{t('waitlist.onListTitle')}</p>
        <p className="text-white/50 text-xs font-bold mt-1">{t('waitlist.position', { n: position })}</p>
      </div>
    );
  }
    ...
    <div className="w-full border-2 border-white/15 p-5 space-y-3">
      ...
      >
        ...
        <span className="uppercase tracking-tighter italic text-sm font-black">
          {submitting ? t('waitlist.joining') : t('waitlist.notifyMe')}
        </span>
      </button>
    </div>
  );
